...
def create_person(name_input, admin_input, project_input, username_input):
    if admin_input.lower() == "yes":
        ...
        admin_input_int = 0
    person_string = (
        '{"name": "%s", "isAdmin": %d, "project": "%s", "username": "%s"}'
        % (name_input, admin_input_int, project_input, username_input)
    )
    ...
    print(
        ...
        % db.people.find_one({"username": username_input}, {"_id": 0})
    )


def update_person(
    username_to_updatef, updated_name, updated_admin, updated_project, updated_username
):
    ...
    for userN in people.find(username_to_updatef):
        ...
        if updated_admin != "":
            db.people.update_one(
                username_to_updatef, {"$set": {"isAdmin": updated_admin_int}}
            )
        if updated_project != "":
            db.people.update_one(
                username_to_updatef, {"$set": {"project": updated_project}}
            )
        if updated_username != "":
            db.people.update_one(
                username_to_updatef, {"$set": {"username": updated_username}}
            )
    ...
    if updated_name == username_to_updatef or updated_name == "":
        ...
        print(
            "Person updated\n %s"
            % db.people.find_one({"username": updated_username}, {"_id": 0})
        )
...
while True:  # Does not exit the code until you give command to exit
    ...
    if selection.lower() == "read":  # Printing everything in people collection
        ...
        selection.lower() == "add document"
        or selection.lower() == "add doc"
        or selection.lower() == "add"
    ):
