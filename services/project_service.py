...
if __name__ == "__main__":
    ...
    zero_option = int(
        input("Please enter 0 in order to adding datas from JSON file to MongoDB:  ")
    )
    if zero_option == 0:
    ...
    while selection != 5:
        if selection == 1:
            ...
            updated_project = str(
                input(
                    "Enter the unique object ID of the information you want to update: "
                )
            )
            ...
            deleted_project = str(
                input("Enter the unique object ID you want to delete: ")
            )
